...
docker run -d -p 15672:15672 -p 5672:5672 \
	...
	rabbitmq
# 运行容器  方式二
docker run -d --hostname myrabbit --name rabbitmq -p 5672:5672 -p 15672:15672 rabbitmq
```
...
登录进去后的管理页面如下：
![image-20240403105223829](D:\MyFiles\markdownFile\images\rabbitManagement.png)
...
```bash
docker run -d -p 3306:3306 --privileged=true -v /usr/local/mysql/log:/var/log/mysql -v /usr/local/mysql/data:/var/lib/mysql -v /usr/local/mysql/conf:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=root --name mysql --restart=always mysql
```
...
mkdir /usr/local/docker/postgresql
mkdir /usr/local/docker/postgresql/data

# 启动容器
docker run --name postgres -e POSTGRES_PASSWORD=root -p 5432:5432 -v /usr/local/postgresql/data:/var/lib/postgresql/data -d postgres
...
> docker run # 创建容器
> -–name postgres # 设置容器名
> -e POSTGRES_PASSWORD=123456 # 设置密码
> -p 5432:5432 # 设置访问端口
> -v /usr/local/postgresql/data:/var/lib/postgresql/data # 将容器中的数据挂载到本地
> -d   : 表示使得容器后台一直运行
>
> postgres:latest #选择镜像
> 
...
mkdir -p /usr/local/nacos/data/					   #新建conf目录
mkdir -p /usr/local/nacos/init.d/					   #新建conf目录
...
```bash
docker run -d --name nacos -p 8848:8848  -p 9848:9848 -p 9849:9849 --privileged=true -e JVM_XMS=256m -e JVM_XMX=256m -e MODE=standalone -v /usr/local/nacos/logs/:/home/nacos/logs -v /usr/local/nacos/conf/:/home/nacos/conf/ --restart=always nacos/nacos-server
```

- 在/usr/local/nacos/init.d/创建custom.properties
...
#配置持久化数据库相关信息 ####################################################
db.num=1
db.url.0=jdbc:mysql://192.168.117.73:3306/nacos?characterEncoding=utf8&connectTimeout=1000&socketTimeout=3000&autoReconnect=true
db.user=root
db.password=root
##########################################################################
...
```shell
docker run --name nacos -d -p 8848:8848 -p 9848:9848 -p 9849:9849 --privileged=true --restart=always -e JVM_XMS=256m -e JVM_XMX=256m -e MODE=standalone -e PREFER_HOST_MODE=hostname -e SPRING_DATASOURCE_PLATFORM=mysql -e MYSQL_SERVICE_HOST=192.168.117.73 -e MYSQL_SERVICE_PORT=3306 -e MYSQL_SERVICE_DB_NAME=nacos -e MYSQL_SERVICE_USER=root -e MYSQL_SERVICE_PASSWORD=root -v /usr/local/nacos/logs:/home/nacos/logs -v /usr/local/nacos/init.d/custom.properties:/etc/nacos/init.d/custom.properties -v  /usr/local/nacos/data:/home/nacos/data --restart=always nacos/nacos-server:1.4.0
...
--
-- Host: hadoop-product-mysql-master-fh-t.seeke.net    Database: nacos
-- ------------------------------------------------------
...
SET @@GLOBAL.GTID_PURGED=/*!80000 '+'*/ '0057264e-8759-11ee-82a5-005056bc84ec:1-386290';
...
-- Dump completed on 2024-02-07 14:45:14
