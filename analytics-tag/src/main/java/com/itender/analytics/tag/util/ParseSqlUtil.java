...
public class ParseSqlUtil {
    ...
    public Map<String, Object> routeSqlParse(TagGroupExpressionRequest expressionRequest, String templateCode) {
        ...
        //根据模板获取每个属性的sql模板位置
        List<AttributeLocation> templateLocation;
        if (Objects.equals(VIP_TAG.getType(), globalTagType)) {
            // 会员标签
            templateLocation = attributeConfCustomVipMapper.getTemplateLocation(templateCode);
        } else {
            templateLocation = attributeConfCustomMapper.getTemplateLocation(templateCode);
        }
        // 模板集合
